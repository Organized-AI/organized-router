# Historical product vision (pre-caching implementation)

This document records the repository's original aspirational design and hosted pricing. It does not describe the currently verified deployment status. See the root README for the runnable gateway.
...
See [DOCUMENTATION/HEALER-PROTOCOL.md](HEALER-PROTOCOL.md) for the wire format.
...
Self-hosters pay neither. That is not a loophole, it is the design. Full detail in [DOCUMENTATION/PRICING.md](PRICING.md).
...
[DOCUMENTATION/DISTRIBUTION.md](DISTRIBUTION.md).
